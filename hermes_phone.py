...
def load_profiles() -> list[dict[str, Any]]:
    ...
    for d in dirs:
        ...
        name = str(ov.get("name") or soul_name or pid)
...
<title>Hermes Lightweight Mobile Desktop</title>
<style>
:root {
  --bg: #0e1116;
  --panel: #161b22;
...
(function () {
  ...
  function render() {
    ...
    title.textContent = bot ? bot.name : "Hermes";
  }
  ...
  async function ensureSession(profileId) {
    ...
    const r = await fetch("/api/new-session", {method: "POST", headers: authHeaders(), body: "{}"});
  ...
  async function send(ev) {
    ...
    try {
      ...
      if (r.status === 429) {
        ...
        const err = document.createElement("div");
        err.className = "bubble err";
        err.textContent = data.error || ("Error " + r.status);
        chat.appendChild(err);
      } else {
...
def main() -> None:
    ...
    sys.stderr.write(
        "Hermes Phone listening on http://%s:%s/  catalog=%s profiles_dir=%s  "
